# ScDowngrade-Lua
A simple lua tool for using [ScDowngrade](https://github.com/Daniil-SV/ScDowngrade) for inexperienced users.
This tool was written in one evening, thanks a lot to my friend [Wicik](https://t.me/wic1k) for helping with the arguments.
The tool works on Windows and Linux.

ScDowngrade By [Daniil SV](https://github.com/Daniil-SV)

[Official ScDowngrade Repository](https://github.com/Daniil-SV/ScDowngrade)

# Linux
installing wine to launch ScDowngrade

Ubuntu
``` sh
sudo apt install wine
```
Arch
``` sh
sudo pacman -S wine
```

Enjoy your use :)


## Authors

- [W1ckedOneshot](https://t.me/W1cked_OneShot)
- [Wic1k](https://t.me/wic1k)

